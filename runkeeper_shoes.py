...
@app.route('/')
def index():
    if 'rk_access_token' in session:
        return redirect(url_for('predict'))
    else:
        url = '/'.join((app.config['BASEURL'], 'login'))
...
def predict():
    ...
    if 'rk_access_token' not in session:
...
<html>
    <body>
        ...
        You bought your running shoes on:
              <input type="date" name="bought"><br>
        Only use date after (optional, if not set will use everything):
    ...
    if 'start' in request.args and \
       re.match('^\d{4}-\d{2}-\d{2}$', request.args['start']):
    ...
    sess = healthgraph.Session(session['rk_access_token'])
    user = healthgraph.User(session=sess)
    act_iter = user.get_fitness_activity_iter()

    dates, dists = zip(*[(act['start_time'], act['total_distance'])
                         for act in act_iter if act['type'] == 'Running'])
    ...
    lr.fit(timestamps.reshape((df.shape[0], 1)), df.cum_dists)
    ...
    points = plt.scatter(timestamps, df.buy_dists, color='b',
                         label='Running sessions (cumulative distance)')
    ...
    plt.legend([buy, points, line, zone],
               ['Shoes bought (on %s)' % buy_date,
                ...
                'Linear regression',
                'Replacement zone (from %s to %s)' % (from_ts(low_x),
                                                      from_ts(high_x))],
               loc='upper right')
...
<html>
   <body>
       ...
       <p><b>Here's how it works</b>: your running data (date and distance for
             every running session you have tracked)
          is first downloaded from RunKeeper
          (with the <a href="http://developer.runkeeper.com/healthgraph"
                       target="_blank">Health Graph API</a>).
          The distances are then transformed into cumulative distances,
          and adjusted in such a way that the day you bought
          your shoes is roughly aligned with a distance of 0. These values are
          then "fitted" (imperfectly) using a
        <a href="http://cjauvin.blogspot.ca/2013/10/linear-regression-101.html"
             target="_blank">linear regression</a>,
          from which the date range where your shoes are predicted to have
          accumulated 500 to 800 kilometers of usage can be read (note that
          these values have been unscientifically gathered and averaged
          from sources like
<a
 href="http://www.runnersworld.com/running-shoes/running-shoe-faq?page=single"
